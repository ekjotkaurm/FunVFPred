...
def calculate_metrics(y_true, y_pred, y_prob, dataset_name):
    ...
    filename = f'RF_AAC_AF_{dataset_name}_metrics.csv'
...
plot_confusion_matrix(test_metrics[-1], ['Negative', 'Positive'], "RF_AAC_test_confusion_matrix.png")
plot_confusion_matrix(validation_metrics[-1], ['Negative', 'Positive'], "RF_AAC_validation_confusion_matrix.png")
...
# Plot ROC curves
plot_roc_curve(y_test, y_test_prob, "RF_AAC_test_roc_curve.png", "Test")
plot_roc_curve(y_validate, y_validate_prob, "RF_AAC_validation_roc_curve.png", "Validation")

# === 6. Save Classification Reports ===
def save_classification_report(y_true, y_pred, dataset_name):
    report = classification_report(y_true, y_pred, target_names=['Negative', 'Positive'])
    with open(f'RF_AAC_{dataset_name}_classification_report.txt', 'w') as f:
        f.write(report)
    print(f"Classification report saved as RF_AAC_{dataset_name}_classification_report.txt.")
